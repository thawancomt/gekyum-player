...
export default function AlbumsTab() {
  ...
  const albunsByInitialLetter = useMemo(() => {
    return Object.keys(albums).reduce(
      (acc, alb) => {
    ...
    <div className=" w-full p-4 h-full overflow-y- overflow-x-hidden flex flex-col items-center">
      <AnimatePresence>
              ...
              .map((alb) => {
                ...
                return (
                  <Button
                    ...
                    onClick={() => toggleInitialLetter(initial)}
                  >
                    {initial}
                  </Button>
                );
        ...
        <Activity mode={show ? "visible" : "hidden"}>
          <motion.div
            ref={gridRef}
            className="h-full w-full    grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 justify-evenly gap-8  overflow-auto no-scrollbar md:p-20 p-4
                                            "
            initial={{
              scale: 0.8,
            }}
            ...
            layout
          >
            {(albunsByInitialLetter[search] || Object.keys(albums || {})).map(
              (alb) => (
                <motion.div layoutId={`album-${alb}`} key={alb}>
                  <AlbumItem
                    name={alb}
                    coverPath={albums[alb]?.[0]?.cover_path}
                    key={alb}
                  />
                </motion.div>
              ),
